...
def open_cifar10(tarball: str, *, max_images: Optional[int]):
    ...
    # Changes for dataset used to train generative model for PANORAMIA 
    all_indices = [i for i in range(len(images))]
    random.shuffle(all_indices)
    gan_indices = all_indices[:int(len(all_indices)/6)]
    images = images.transpose([0, 2, 3, 1]) # NCHW -> NHWC
    images = images[gan_indices]
